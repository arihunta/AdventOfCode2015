...
public class Day07 {
	...
	enum Operation { AND, OR, LSHIFT, RSHIFT, NOT, VALUE };
    ...
    private static int valueOf(final String gateId, final Map<String, Gate> circuit, final Map<String, Integer> valueCache) {
		if (valueCache.containsKey(gateId)) return valueCache.get(gateId);
    	if (circuit.containsKey(gateId)) {
			var gate = circuit.get(gateId);
    		var value = switch (gate.operation) {
				case AND -> valueOf(gate.inputs.get(0), circuit, valueCache) & valueOf(gate.inputs.get(1), circuit, valueCache);
				case OR -> valueOf(gate.inputs.get(0), circuit, valueCache) | valueOf(gate.inputs.get(1), circuit, valueCache);
				case LSHIFT -> valueOf(gate.inputs.get(0), circuit, valueCache) << valueOf(gate.inputs.get(1), circuit, valueCache);
				case RSHIFT -> valueOf(gate.inputs.get(0), circuit, valueCache) >> valueOf(gate.inputs.get(1), circuit, valueCache);
